...
//#if IMGUI_VERSION_NUM >= 19135
        return (CImGui::GetCurrentContext()->LastItemData.ItemFlags & ImGuiItemFlags_MixedValue) != 0;
//#else
//        return (CImGui::GetCurrentContext()->LastItemData.InFlags & ImGuiItemFlags_MixedValue) != 0;
//#endif
    }
} // namespace
...
{
    ImGuiWindow* window = CImGui::GetCurrentWindow();
    ...
    // update igui context
    ImGuiContext& g = *CImGui::GetCurrentContext();
    _id = CImGui::ImGuiWindowUtils::GetID(window, _label);
    _drawList = CImGui::GetWindowDrawList();
    _style = &CImGui::GetStyle();
    ...
    ImVec2 label_size = CImGui::CalcTextSize(_label, nullptr, true);
...
{
    assert(_config.Size.x >= 0 && "Size.x specified was negative.");
    assert(_config.Size.y >= 0 && "Size.y specified was negative.");
...
{
    CImGui::ItemSize(interaction_bounding_box, _style->FramePadding.y);
    if (!CImGui::ItemAdd(interaction_bounding_box, _id))
    {
        ImGuiContext& g = *CImGui::GetCurrentContext();
        return false;
    }
    ...
    bool hovered, held;
    bool pressed = CImGui::ButtonBehavior(interaction_bounding_box, _id, &hovered, &held, button_flags);
    if (pressed)
    {
        *_value = !(*_value);
        CImGui::MarkItemEdited(_id);
    }
...
void ImGuiToggleRenderer::DrawToggle()
{
    ...
    const ImU32 color_frame = CImGui::GetColorU32(_isHovered ? _palette.FrameHover : _palette.Frame);
    const ImU32 color_knob = CImGui::GetColorU32(_isHovered ? _palette.KnobHover : _palette.Knob);
...
void ImGuiToggleRenderer::DrawFrame(ImU32 color_frame)
{
    ...
    if (HasShadowedFrame())
    {
        const ImU32 color_frame_shadow = CImGui::GetColorU32(_palette.FrameShadow);
    ...
    if (HasBorderedFrame())
    {
        const ImU32 color_frame_border = CImGui::GetColorU32(_palette.FrameBorder);
...
{
    CImGui::RenderBullet(_drawList->drawList, pos, color);
}
...
void ImGuiToggleRenderer::DrawA11yLabel(ImVec2 pos, ImU32 color, const char* label)
{
    ...
    const ImVec2 text_size = CImGui::CalcTextSize(label);
...
void ImGuiToggleRenderer::DrawA11yFrameOverlay(float knob_radius, bool state)
{
    ...
    const float thickness = std::ceil(radius * 0.2f);
    ...
    const ImU32 color = state
        ? CImGui::GetColorU32(_colorA11yGlyphOn)
        : CImGui::GetColorU32(_colorA11yGlyphOff);
...
void ImGuiToggleRenderer::DrawCircleKnob(float radius, ImU32 color_knob)
{
    ...
    assert(inset_size <= radius && "Inset size needs to be smaller or equal to the knob's radius for circular knobs.");
    ...
    if (HasShadowedKnob())
    {
        const ImU32 color_knob_shadow = CImGui::GetColorU32(_palette.KnobShadow);
    ...
    if (HasBorderedKnob())
    {
        const ImU32 color_knob_border = CImGui::GetColorU32(_palette.KnobBorder);
...
void ImGuiToggleRenderer::DrawRectangleKnob(float radius, ImU32 color_knob)
{
    ...
    const float knob_diameter_total = ImRectUtils::GetHeight(bounds);
...
{
    const ImVec2 label_size = CImGui::CalcTextSize(_label, nullptr, true);
    ...
    if (g.LogEnabled)
    {
        CImGui::LogRenderedText(&label_pos, _isMixedValue ? "[~]" : *_value ? "[x]" : "[ ]");
    }

    if (label_size.x > 0.0f)
    {
        CImGui::RenderText(label_pos, _label);
    }
}
...
void ImGuiToggleRenderer::UpdatePalette()
{
    ...
    CImGui::UnionPalette(&off_unioned, off_candidate, _style->Colors, false);
    CImGui::UnionPalette(&on_unioned, on_candidate, _style->Colors, true);

    // otherwise, lets lerp them!
    CImGui::BlendPalettes(&_palette, off_unioned, on_unioned, _animationPercent);
...
void ImGuiToggleRenderer::DrawRectBorder(ImRect bounds, ImU32 color_border, float rounding, float thickness)
{
    ...
    const float half_thickness = thickness * 0.5f;
    ImRectUtils::Expand(bounds, -half_thickness);
