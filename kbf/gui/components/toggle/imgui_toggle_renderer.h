...
private:
    ...
    std::unique_ptr<CImGui::ImDrawListTransparent> _drawList;
    ImGuiID _id;
    ...
    // inline accessors
    inline float GetWidth() const { return ImRectUtils::GetWidth(_boundingBox); }
    inline float GetHeight() const { return ImRectUtils::GetHeight(_boundingBox); }
    inline ImVec2 GetPosition() const { return _boundingBox.Min; }
    inline ImVec2 GetToggleSize() const { return ImRectUtils::GetSize(_boundingBox); }
